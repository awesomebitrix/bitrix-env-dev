...
* **_-e NOMYSQL=1_** если определен этот параметр, то контейнер не будет работать со встроенным сервером MySQL; это для тех, кто хочет держать базу в другом контейнере (отличный вариант, кстати)
