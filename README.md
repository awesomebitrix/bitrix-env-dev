...
Если вы хотите работать с файлами проекта извне, то самым верным способом будет использование `ssh/sftp` от имени `bitrix`. 
...
А еще мы вынесем директорию mysql в хост-систему. Это позволит сохранить базы между перезапусками mysql-контейнера.
